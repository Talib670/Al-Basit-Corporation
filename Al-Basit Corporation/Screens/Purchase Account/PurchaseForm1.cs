...
{
    public partial class PurchaseForm1 : Form4
    {
        productDataClasses1DataContext pd = new productDataClasses1DataContext();
        bqayaproductDataClasses1DataContext bd = new bqayaproductDataClasses1DataContext();
        public PurchaseForm1()
        {
            InitializeComponent();
        }

        private void PurchaseForm1_Load(object sender, EventArgs e)
        {
            ...
            label5.Visible = false;
            {
                SqlConnection conn = new SqlConnection(DecHelper.ConnectionString);
                conn.Open();
                string str = "  Select * FROM [Albbasitcorporation].[dbo].[productbqayaamount]";
        ...
        {
            {
                SqlConnection conn = new SqlConnection(DecHelper.ConnectionString);
                conn.Open();
                string str = "  Select * FROM [Albbasitcorporation].[dbo].[productbqayaamount] WHERE [nameproduct]= '" + comboBox1.Text + "'";
                SqlCommand cmd = new SqlCommand(str, conn);
                SqlDataReader da = cmd.ExecuteReader();
                while (da.Read())
                {
...
SET [Grand Total] = '" + total.Text + "'";
                ...
                pd.tblproducts.InsertOnSubmit(t);
                pd.SubmitChanges();
...

                {
                    ...
                    string str = @" UPDATE [Albbasitcorporation].[dbo].[productbqayaamount]
SET [bqayaamount]= '" + bqayatextBox1.Text + "'  WHERE [nameproduct] = '" + comboBox1.Text + "'";
                ...
                this.Hide();
                PurchaseForm1 i = new PurchaseForm1();
                i.ShowDialog();
            }
        }
        ...
        {
            TotalpurchaseForm1 tf = new TotalpurchaseForm1();
            tf.ShowDialog();
        }

        private void PurchaseForm1_KeyDown(object sender, KeyEventArgs e)
        {
